...
class OllamaAgent(conversation.AbstractConversationAgent):
    ...
    async def async_process(
        ...
        if response.get("done_reason", "") == "stop":
            ...
            if "tool_calls" in assistant_message:
                await self._handle_tool_calls(messages, assistant_message, user_input, conversation_id)
            else:
    ...
    async def _handle_tool_calls(self, messages, assistant_message, user_input: conversation.ConversationInput, conversation_id):
        ...
        for tool_call in assistant_message.get("tool_calls", []):
            ...
            # Share results of the tool call with the AI assistant
            try:
                response = await self.query(messages)
            except (
                ApiCommError,
